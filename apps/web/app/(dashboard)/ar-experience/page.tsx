...
import {
  ...
  Package, Smartphone, ClipboardList, Search, Hammer, Palette, Home,
  ...
  return (
    <div style={{ padding: 0 }}>
      ...
      <div style={{ display: 'flex', gap: 4, background: surf, padding: 4, borderRadius: 12, width: 'fit-content', marginBottom: 20, border: '1px solid #e2e8f0' }}>
        {[{ id: 'models', label: `Modellerim (${models.length})`, Icon: Target }, { id: 'upload', label: t('Model Yükle','Model Yükle'), Icon: Plus }].map(tb => (
          <button key={tb.id} onClick={() => setTab(tb.id as any)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 20px', borderRadius: 9, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, transition: 'all 0.2s', background: tab === tb.id ? 'linear-gradient(135deg,#be185d,#7c3aed)' : 'transparent', color: tab === tb.id ? '#fff' : tx2, boxShadow: tab === tb.id ? '0 4px 16px rgba(190,24,93,0.3)' : 'none' }}>
            <tb.Icon size={14} />
            {tb.label}
          </button>
        ))}
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 4, background: surf, padding: 4, borderRadius: 12, width: 'fit-content', marginBottom: 20, border: '1px solid #e2e8f0' }}>
            {[
              ...
              <button key={m.id} onClick={() => setUploadMode(m.id as any)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 20px', borderRadius: 9, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, lineHeight: 1.4, transition: 'all 0.2s', background: uploadMode === m.id ? (m.id === 'ai' ? 'linear-gradient(135deg,#7c3aed,#4f46e5)' : 'linear-gradient(135deg,#be185d,#7c3aed)') : 'transparent', color: uploadMode === m.id ? '#fff' : tx2, boxShadow: uploadMode === m.id ? '0 4px 16px rgba(124,58,237,0.3)' : 'none' }}>
                <m.Icon size={15} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              <div style={{ ...card, padding: 24 }}>
                ...
                <div onDragOver={e => { e.preventDefault(); setIsDragging(true) }} onDragLeave={() => setIsDragging(false)}
                  ...
                  <p style={{ color: tx1, fontWeight: 600, fontSize: 14, margin: '0 0 4px' }}>{uploading ? 'Yükleniyor...' : isDragging ? 'Bırakın!' : '.glb veya .usdz dosyasını buraya sürükleyin'}</p>
                  <p style={{ color: tx2, fontSize: 12, margin: 0 }}>Maksimum 50MB</p>
                </div>
                <button onClick={() => !uploading && fileRef.current?.click()} disabled={uploading}
                  style={{ width: '100%', marginTop: 10, padding: '11px 16px', borderRadius: 10, border: '1px solid rgba(236,72,153,0.35)', cursor: uploading ? 'not-allowed' : 'pointer', background: 'rgba(236,72,153,0.08)', color: '#db2777', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'background 0.2s' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 20 }}>
                  <div style={{ ...card, border: '1px solid #ede9fe', padding: 24 }}>
                    ...
                    {(!aiPhotos[0] || !productName) && (
                      <p style={{ color: tx2, fontSize: 12, textAlign: 'center', marginTop: 8 }}>{!productName ? 'Ürün adı yazın ve ' : ''}ön cephe fotoğrafı ekleyin</p>
                    )}
                  </div>
              ...
              {aiScreen === 'generating' && (
                <div style={{ ...card, border: '1px solid #ede9fe', padding: '48px 32px', textAlign: 'center', minHeight: 360 }}>
                      ...
                      return (
                        <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                          ...
                          <p style={{ color: isDone ? accentEmerald : isActive ? '#7c3aed' : tx3, fontSize: 13, fontWeight: isActive ? 600 : 400, margin: 0 }}>
                            {label}{isActive && <span style={{ color: '#7c3aed' }}>{'.'.repeat(dotCount)}</span>}
                          </p>
                        </div>
                      )
                    })}
                  ...
                  <div style={{ maxWidth: 480, margin: '0 auto', height: 6, background: '#e2e8f0', borderRadius: 6, overflow: 'hidden' }}>
                    <div style={{ height: '100%', borderRadius: 6, transition: 'width 0.5s ease', width: `${Math.max(5, aiProgress)}%`, background: 'linear-gradient(90deg,#7c3aed,#ec4899)', boxShadow: '0 0 12px rgba(124,58,237,0.4)' }} />
                  </div>
                  <p style={{ color: tx2, fontSize: 12, marginTop: 10 }}>{aiProgress}% tamamlandı</p>
                </div>
              )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    <div style={{ ...card, border: '1px solid rgba(5,150,105,0.2)', padding: 24 }}>
                      ...
                      <div style={{ background: surf, borderRadius: 10, padding: '12px 14px', marginBottom: 20, border: '1px solid #e2e8f0' }}>
                        ...
                        <p style={{ color: tx1, fontSize: 14, fontWeight: 600, margin: 0 }}>{productName || 'AI Oluşturuldu'}</p>
                      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ ...card, padding: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                <label style={{ color: tx2, fontSize: 12, display: 'block', marginBottom: 6 }}>Toplu Seçim ({selectedLeads.length} seçili)</label>
