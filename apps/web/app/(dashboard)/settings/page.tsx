        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', gap:16 }}>
          ...
          {tab === 'google-capi' && (
            <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              ...
              {/* Connection — always show connect/reconnect */}
              <div style={{ background: gcapi.hasConnection ? 'linear-gradient(135deg, #f0fdf4, #ecfdf5)' : 'linear-gradient(135deg, #eff6ff, #f0fdf4)', border: `1px solid ${gcapi.hasConnection ? '#86efac' : '#93c5fd'}`, borderRadius:14, padding:'14px 18px' }}>
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
                  <span style={{ fontSize:18 }}>🔗</span>
                  <div style={{ flex:1 }}>
                    <p style={{ color:'#0f172a', fontSize:13, fontWeight:700, margin:0 }}>{gcapi.hasConnection ? 'Google Ads Bağlı' : 'Google Ads Bağlayın'}</p>
                    <p style={{ color:'#64748b', fontSize:11, margin:'2px 0 0' }}>{gcapi.hasConnection ? 'Bağlantıyı yenilemek için tekrar bağlanın' : 'Conversion tracking için Google hesabınızı bağlayın'}</p>
                  </div>
                </div>
                <button onClick={async () => {
                  try {
                    const d = await api.get('/api/google-ads/oauth-url')
                    if (d.url) window.location.href = d.url
                  } catch { window.location.href = '/google-ads' }
                }} style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, width:'100%', padding:'10px 16px', background:'#4285F4', color:'#fff', border:'none', borderRadius:10, fontSize:13, fontWeight:700, cursor:'pointer' }}>
                  ...
                  {gcapi.hasConnection ? 'Tekrar Bağlan' : 'Google ile Bağlan'}
                </button>
              </div>
