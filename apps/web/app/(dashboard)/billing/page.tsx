...
export default function BillingPage() {
  ...
  const handlePurchase = async (packageId: string) => {
    ...
    try {
      const data = await api.post('/api/billing/topup', { packageId })
  ...
  const remaining = (user?.creditsTotal || 0) - (user?.creditsUsed || 0)
  const percent = user?.creditsTotal ? (remaining / user.creditsTotal) * 100 : 0
    ...
    <div className="space-y-8">
      ...
      <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-slate-400 text-sm">/ {user?.creditsTotal} toplam</p>
        </div>
        ...
        {remaining < 20 && (
          <p className="text-yellow-400 text-sm mt-3">⚠️ Krediniz azalıyor, paket satın almayı unutmayın!</p>
        )}
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {packages.map(pkg => (
            <div key={pkg.id} className={`relative bg-slate-800/50 border rounded-xl p-6 ${pkg.popular ? 'border-blue-500' : 'border-slate-700'}`}>
              {pkg.popular && (
