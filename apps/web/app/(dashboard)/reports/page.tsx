...
import { RefreshCw, Download, ArrowUpRight, ArrowDownRight, Users, Trophy, Target, DollarSign, BarChart3, CheckCircle, Zap } from 'lucide-react'
            ...
            <div>
              ...
              <div style={{ display:'flex', gap:6 }}>
                {(['weekly','monthly'] as const).map(v => (
                  <button key={v} onClick={() => setView(v)} style={{ padding:'6px 18px', borderRadius:20, border:`1px solid ${view===v?'rgba(124,58,237,0.4)':'#e2e8f0'}`, background:view===v?'rgba(124,58,237,0.1)':'transparent', color:view===v?'#7c3aed':'#475569', fontSize:12, fontWeight:600, cursor:'pointer' }}>
      ...
      {loading ? (
        ...
        <>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:20 }}>
            {[
              { label:'Yeni Lead', value:d.newLeads, color:'#8b5cf6', Icon: Users, trend:d.prevNewLeads?Math.round(((d.newLeads-d.prevNewLeads)/(d.prevNewLeads||1))*100):0 },
              { label: t('Kazanılan','Kazanılan'), value:d.wonLeads, color:'#10b981', Icon: Trophy, trend:0 },
              { label: t('Dönüşüm','Dönüşüm'), value:`%${d.conversionRate}`, color:'#06b6d4', Icon: Target },
              { label:'Gelir', value:fmtCurrency(d.totalRevenue||0), color:'#f59e0b', Icon: DollarSign },
            ].map(m => (
