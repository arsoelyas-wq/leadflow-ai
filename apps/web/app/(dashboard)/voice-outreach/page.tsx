...
import {
  ...
  Globe2, Search, Filter, Star, Users, BarChart2,
  Settings, ArrowRight, X, ChevronDown, Sparkles
} from 'lucide-react'
...
function VoiceCard({ voice, selected, onSelect, onPreview, playing }: any) {
  ...
  const sourceColor = voice.source === 'my' ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-700 text-slate-400'

  return (
    <div onClick={() => onSelect(voice.voice_id, voice.name)}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
          {voice.gender && <span className="text-xs text-slate-500">{voice.gender === 'female' ? 'Kadın' : 'Erkek'}</span>}
          ...
          {voice.source === 'my' && <span className={`text-xs px-1.5 py-0.5 rounded-md ${sourceColor}`}>Benim</span>}
        </div>
      </div>
      {/* Play button */}
      <button onClick={e => { e.stopPropagation(); onPreview(voice.voice_id) }}
...
// ── SES SEÇİM PANELİ (ElevenLabs gibi) ──────────────────
function VoiceLibrary({ selectedId, selectedName, onSelect, previewLang }: any) {
  ...
  async function loadVoices() {
    setLoading(true)
    try {
      const r = await fetch(`${API}/api/voice/eleven-voices`, { headers: authH() })
  ...
  const tabs = [
    { key: 'turkish', label: '🇹🇷 Türkçe', count: voices.turkish?.length },
    { key: 'professional', label: '💼 Profesyonel', count: voices.professional?.length },
    { key: 'my', label: '⭐ Seslerim', count: voices.my?.length },
    { key: 'cloned', label: '🎤 Klonlanmış', count: voices.cloned?.length },
    { key: 'all', label: '🌍 Tüm Sesler', count: voices.all?.length },
  ]
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="flex flex-col items-center py-12 gap-3">
          ...
          <p className="text-slate-500 text-sm">Ses kütüphanesi yükleniyor...</p>
        </div>
      ) : currentList.length === 0 ? (
        <div className="text-center py-12 text-slate-600">
          <Volume2 className="w-10 h-10 mx-auto mb-2 opacity-30"/>
          <p className="text-sm">
            {activeTab === 'cloned' ? 'Klonlanmış ses yok. Aşağıdan kendi sesinizi ekleyin.' : 'Ses bulunamadı'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-2 max-h-[400px] overflow-y-auto pr-1 scrollbar-hide">
...
export default function VoicePage() {
  const [tab, setTab] = useState<'dial' | 'campaign' | 'calls' | 'voice' | 'settings'>('voice')
  ...
  async function cloneVoice() {
    ...
    try {
      ...
      const d = await r.json()
      if (d.ok) { showMsg('success', '🎤 Sesiniz başarıyla klonlandı!'); loadAll() }
  ...
  async function makeSingleCall() {
    ...
    if (!selectedVoiceId) return showMsg('error', 'Önce ses seçin')
    setCalling(true)
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">ElevenLabs + Claude Sonnet · %100 insan sesi · 12 dil · Türkiye & İhracat</p>
        </div>
        {selectedVoiceName ? (
          <div className="flex items-center gap-2 px-3 py-2 bg-teal-500/10 border border-teal-500/20 rounded-xl cursor-pointer" onClick={() => setTab('voice')}>
            ...
            <span className="text-teal-600 text-xs">değiştir</span>
          </div>
        ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 bg-slate-800/50 border border-slate-700 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2 items-center">
                <span className="text-xs text-slate-500">Önizleme dili:</span>
          ...
          <div className="space-y-4">
            <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-5 space-y-4">
              ...
              <p className="text-slate-400 text-sm leading-relaxed">30 saniye — 3 dakika net ses kaydı yükleyin. Sistem tam olarak sizin sesinizle arama yapar.</p>
              ...
              <button onClick={cloneVoice} disabled={cloning}
                ...
                {cloning ? <><RefreshCw className="w-4 h-4 animate-spin"/> Klonlanıyor...</> : <><Mic className="w-4 h-4"/> Sesimi Klonla</>}
              </button>
            </div>
      ...
      {tab === 'dial' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-5 space-y-4">
            ...
            {!selectedVoiceName && (
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center gap-2">
                ...
                <span className="text-amber-300 text-xs">Önce Sesler sekmesinden ses seçin</span>
                <button onClick={() => setTab('voice')} className="ml-auto text-xs text-amber-400 underline">Ses Seç</button>
              </div>
            )}
            ...
            <div className="bg-slate-900 rounded-xl p-3 text-xs text-slate-400 space-y-1">
              <p>→ ElevenLabs + {selectedVoiceName || 'Seçili Ses'} ile arama</p>
              <p>→ {LANGUAGES[selectedLanguage]?.flag} {LANGUAGES[selectedLanguage]?.name} dilinde konuşacak</p>
              <p>→ Claude Sonnet konuşmayı yönetecek</p>
            </div>

            <button onClick={makeSingleCall} disabled={calling || !selectedLead || !selectedVoiceId}
          ...
          <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-5">
            ...
            <div className="space-y-4">
              {[
                { step: '1', title: 'Ses Seç', desc: 'ElevenLabs kütüphanesinden ses seç veya kendi sesini klonla', color: 'bg-teal-500/20 text-teal-400' },
                { step: '2', title: 'Lead & Dil', desc: 'Aranacak leadi ve konuşma dilini seç', color: 'bg-blue-500/20 text-blue-400' },
                { step: '3', title: 'Arama Başlar', desc: 'ElevenLabs AI seçilen sesle müşteriyi arar', color: 'bg-purple-500/20 text-purple-400' },
                { step: '4', title: 'Claude Yönetir', desc: 'Konuşmayı Claude Sonnet yönetir, satış yapar', color: 'bg-amber-500/20 text-amber-400' },
                { step: '5', title: 'Analiz Gelir', desc: 'Arama bittikten sonra transcript ve analiz LeadFlow\'a gelir', color: 'bg-emerald-500/20 text-emerald-400' },
              ].map(s => (
      ...
      {tab === 'campaign' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 bg-slate-800/50 border border-slate-700 rounded-2xl p-5 space-y-4">
            ...
            <div className="bg-slate-900 rounded-xl p-3 text-xs text-slate-400 space-y-1">
              <p>→ Her lead ülkesine göre dil otomatik seçilir</p>
              <p>→ Tahmini süre: ~{Math.round(selectedLeads.length * (delayMinutes + 3))} dakika</p>
            </div>

            <button onClick={startCampaign} disabled={campaignRunning || !selectedLeads.length || !selectedVoiceId}
              ...
              {campaigns.map(c => (
                <div key={c.id} className="p-3 bg-slate-900 rounded-xl">
                  ...
                  <div className="mt-1.5 h-1 bg-slate-700 rounded-full">
                    <div className="h-full bg-teal-500 rounded-full transition-all" style={{ width: `${c.total_leads ? (c.calls_made/c.total_leads)*100 : 0}%` }}/>
                  </div>
                </div>
              ))}
