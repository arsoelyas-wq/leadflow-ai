...
function ProductCard({ p, onDelete, onToggle, onEdit }: { p: Product; onDelete: (id: string) => void; onToggle: (id: string, v: boolean) => void; onEdit: (p: Product) => void }) {
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg,rgba(5,8,22,0.97),rgba(8,6,20,0.99))', border: `1px solid ${p.is_active ? 'rgba(99,102,241,0.2)' : 'rgba(71,85,105,0.15)'}`, borderRadius: 16, overflow: 'hidden', opacity: p.is_active ? 1 : 0.55, transition: 'opacity 0.2s' }}>
      {/* Image */}
      <div style={{ height: 120, background: 'rgba(0,0,0,0.4)', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {p.images && p.images[0] ? (
        ...
        {p.images && p.images[1] && (
          <img src={p.images[1]} alt="" style={{ position: 'absolute', bottom: 4, right: 4, width: 36, height: 36, objectFit: 'cover', borderRadius: 6, border: '1px solid rgba(255,255,255,0.15)' }} />
        )}
        {p.category && (
          <span style={{ position: 'absolute', top: 6, left: 6, background: 'rgba(0,0,0,0.7)', color: '#a5b4fc', fontSize: 10, padding: '2px 8px', borderRadius: 12, fontWeight: 600 }}>{p.category}</span>
        )}
      </div>
      {/* Content */}
      <div style={{ padding: '14px 16px' }}>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: 14, margin: '0 0 4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</p>
        {p.price && <p style={{ color: '#a5b4fc', fontWeight: 700, fontSize: 13, margin: '0 0 6px' }}>{p.price}</p>}
        {p.description && <p style={{ color: '#475569', fontSize: 11, margin: '0 0 10px', lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' } as any}>{p.description}</p>}
        {p.specs && <p style={{ color: '#334155', fontSize: 10, margin: '0 0 10px', lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical', overflow: 'hidden' } as any}>📋 {p.specs}</p>}
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <button onClick={() => onEdit(p)} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, padding: '6px 8px', borderRadius: 7, border: 'none', cursor: 'pointer', background: 'rgba(99,102,241,0.12)', color: '#a5b4fc', fontSize: 11 }}><Edit3 size={11} /> Düzenle</button>
          <button onClick={() => onToggle(p.id, p.is_active)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 8px', borderRadius: 7, border: 'none', cursor: 'pointer', background: p.is_active ? 'rgba(16,185,129,0.1)' : 'rgba(71,85,105,0.1)', color: p.is_active ? '#34d399' : '#64748b', fontSize: 11 }}>{p.is_active ? <ToggleRight size={11} /> : <ToggleLeft size={11} />}</button>
          {!confirm ? (
            <button onClick={() => setConfirm(true)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 8px', borderRadius: 7, border: 'none', cursor: 'pointer', background: 'rgba(127,29,29,0.25)', color: '#fca5a5', fontSize: 11 }}><Trash2 size={11} /></button>
          ) : (
            <button onClick={() => onDelete(p.id)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 10px', borderRadius: 7, border: 'none', cursor: 'pointer', background: 'rgba(239,68,68,0.2)', color: '#f87171', fontSize: 11, fontWeight: 700 }}>Sil?</button>
          )}
        </div>
      </div>
    </div>
...
function ProductForm({ initial, onClose, onSaved, token }: { initial?: Product | null; onClose: () => void; onSaved: () => void; token: string }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: 'linear-gradient(135deg,rgba(5,8,22,0.99),rgba(8,6,20,0.99))', border: '1px solid rgba(99,102,241,0.25)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
          <h2 style={{ color: '#fff', fontSize: 16, fontWeight: 700, margin: 0 }}>{initial ? '✏️ Ürün Düzenle' : '➕ Yeni Ürün Ekle'}</h2>
          <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: '50%', border: 'none', background: 'rgba(255,255,255,0.06)', color: '#64748b', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={14} /></button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 18 }}>
          {[0, 1].map(i => (
            <div key={i}>
              <label style={{ color: '#64748b', fontSize: 11, display: 'block', marginBottom: 6 }}>{i === 0 ? 'Ana Görsel *' : 'İkinci Görsel (opsiyonel)'}</label>
              {previews[i] ? (
                <div style={{ position: 'relative', height: 110, borderRadius: 10, overflow: 'hidden' }}>
                  ...
                  <button onClick={() => removeImage(i)} style={{ position: 'absolute', top: 5, right: 5, width: 22, height: 22, borderRadius: '50%', border: 'none', background: 'rgba(0,0,0,0.7)', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11 }}>✕</button>
                </div>
              ) : (
                <div onClick={() => (i === 0 ? fileRef0 : fileRef1).current?.click()}
                  style={{ height: 110, borderRadius: 10, border: '1.5px dashed rgba(99,102,241,0.3)', background: 'rgba(99,102,241,0.04)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', gap: 6 }}>
                  <ImageIcon size={20} style={{ color: '#334155' }} />
                  <span style={{ color: '#334155', fontSize: 11 }}>Görsel yükle</span>
                </div>
              )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          <div style={{ gridColumn: 'span 2' }}>
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="örn: Compact LED Masa Lambası"
              style={{ width: '100%', background: '#07091c', border: `1px solid ${name ? 'rgba(99,102,241,0.4)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 9, padding: '10px 13px', color: '#fff', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <div>
            <label style={{ color: '#64748b', fontSize: 11, display: 'block', marginBottom: 5 }}>Fiyat</label>
            <input value={price} onChange={e => setPrice(e.target.value)} placeholder="₺499 / $25 / Teklif"
              style={{ width: '100%', background: '#07091c', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 9, padding: '10px 13px', color: '#fff', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <div>
            <label style={{ color: '#64748b', fontSize: 11, display: 'block', marginBottom: 5 }}>Kategori</label>
            <div style={{ position: 'relative' }}>
              <select value={category} onChange={e => setCategory(e.target.value)}
                style={{ width: '100%', background: '#07091c', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 9, padding: '10px 13px', color: category ? '#fff' : '#64748b', fontSize: 13, outline: 'none', appearance: 'none', cursor: 'pointer' }}>
          ...
          <div style={{ gridColumn: 'span 2' }}>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} placeholder="Ürün hakkında detaylı açıklama — AI bu bilgiyi kullanarak müşterilere yanıt verir"
              style={{ width: '100%', background: '#07091c', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 9, padding: '10px 13px', color: '#fff', fontSize: 13, outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }} />
          </div>
          <div style={{ gridColumn: 'span 2' }}>
            <label style={{ color: '#64748b', fontSize: 11, display: 'block', marginBottom: 5 }}>Teknik Özellikler</label>
            <input value={specs} onChange={e => setSpecs(e.target.value)} placeholder="örn: 40W, 220V, IP65, 3000K sıcak beyaz, A++ enerji"
              style={{ width: '100%', background: '#07091c', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 9, padding: '10px 13px', color: '#fff', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
          </div>
        </div>

        {error && <p style={{ color: '#f87171', fontSize: 12, margin: '0 0 12px', background: 'rgba(239,68,68,0.08)', padding: '8px 12px', borderRadius: 8 }}>{error}</p>}
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
          ...
          <button onClick={onClose} style={{ padding: '11px 18px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)', background: 'transparent', color: '#64748b', fontSize: 14, cursor: 'pointer' }}>İptal</button>
        </div>
      </div>
    </div>
  )
...
export default function ProductsPage() {
  ...
  return (
    <div style={{ padding: 0 }}>
      ...
      {/* ── HERO ──────────────────────────────────────────────────────────── */}
      <div style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg,rgba(5,5,30,0.98),rgba(8,5,22,0.98))', borderRadius: 20, padding: '32px 28px', marginBottom: 24, border: '1px solid rgba(99,102,241,0.15)' }}>
        ...
        <div style={{ position: 'absolute', top: -60, right: -40, width: 240, height: 240, background: 'radial-gradient(circle,rgba(99,102,241,0.1) 0%,transparent 70%)', zIndex: 0 }} />
        <div style={{ position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexShrink: 0 }}>
            ...
            <button onClick={() => { setEditProduct(null); setShowForm(true) }}
              style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 20px', borderRadius: 11, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff', fontSize: 13, fontWeight: 700, boxShadow: '0 6px 20px rgba(79,70,229,0.35)', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <button onClick={loadAiContext}
          style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', borderRadius: 11, border: '1px solid rgba(139,92,246,0.3)', cursor: 'pointer', background: 'rgba(139,92,246,0.08)', color: '#c4b5fd', fontSize: 13, fontWeight: 600 }}>
          <Brain size={15} /> AI Bilgi Bankasını Görüntüle
        </button>
        {showAI && aiContext && (
          <div style={{ marginTop: 12, background: 'rgba(5,8,22,0.97)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: 14, padding: '18px 20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              <p style={{ color: '#a78bfa', fontSize: 13, fontWeight: 600, margin: 0 }}>🤖 AI'nın ürün bilgisi ({aiContext.productCount} ürün)</p>
              <button onClick={() => setShowAI(false)} style={{ background: 'none', border: 'none', color: '#475569', cursor: 'pointer' }}><X size={14} /></button>
            </div>
            <pre style={{ color: '#64748b', fontSize: 11, lineHeight: 1.6, margin: 0, whiteSpace: 'pre-wrap', fontFamily: 'monospace' }}>{aiContext.context || 'Henüz ürün eklenmemiş — ürün ekledikten sonra AI bu bilgileri kullanacak.'}</pre>
          </div>
        )}
      </div>
      ...
      {products.length > 0 && (
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          {(search || filterCat) && (
            <button onClick={() => { setSearch(''); setFilterCat('') }} style={{ padding: '9px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)', background: 'transparent', color: '#64748b', fontSize: 12, cursor: 'pointer' }}>Temizle</button>
          )}
        </div>
      )}
      ...
      {loading ? (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 120 }}>
          <RefreshCw size={24} style={{ color: '#475569', animation: 'prodSpin 1s linear infinite' }} />
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div>
          <p style={{ color: '#475569', fontSize: 12, margin: '0 0 14px' }}>{filtered.length} ürün{search || filterCat ? ` (${products.length} toplam)` : ''}</p>
