...
function buildEmailHtml(report: any) {
  ...
  const benchmarkText = vsbenchmark >= 0 ? `Sektör ortalamasının ${vsbenchmark} puan üstünde` : `Sektör ortalamasının ${Math.abs(vsbenchmark)} puan altında`;
  ...
  const memberRows = report.memberSummary.map((m: any) => `
    <tr style="border-bottom:1px solid #1e293b">
      ...
      <td style="padding:10px;text-align:center">
        ${m.avg_score ? `<span style="background:${m.avg_score >= 80 ? '#065f46' : m.avg_score >= 60 ? '#78350f' : '#7f1d1d'};color:${m.avg_score >= 80 ? '#10b981' : m.avg_score >= 60 ? '#f59e0b' : '#ef4444'};padding:3px 10px;border-radius:20px;font-weight:700">${m.avg_score}</span>` : '<span style="color:#475569">—</span>'}
      </td>
    </tr>
  `).join('');
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>LeadFlow Haftalık Rapor</title></head>
<body style="margin:0;padding:0;background:#0f172a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px">
    ...
    <div style="background:linear-gradient(135deg,#6d28d9,#4f46e5);border-radius:16px;padding:32px;margin-bottom:24px;text-align:center">
      ...
      <h1 style="color:#fff;margin:0;font-size:24px;font-weight:700">Haftalık Ekip Raporu</h1>
      <p style="color:#c4b5fd;margin:8px 0 0">LeadFlow AI · ${new Date().toLocaleDateString('tr-TR', {day:'numeric',month:'long',year:'numeric'})}</p>
    </div>

    <!-- Ana Metrik -->
    <div style="background:#1e293b;border-radius:16px;padding:24px;margin-bottom:16px;text-align:center">
      <div style="font-size:56px;font-weight:800;color:${report.avgScore >= 80 ? '#10b981' : report.avgScore >= 60 ? '#f59e0b' : '#ef4444'}">${report.avgScore || '—'}</div>
      <div style="color:#94a3b8;font-size:14px;margin-top:4px">Haftalık Ortalama Skor</div>
      <div style="margin-top:12px;display:flex;justify-content:center;gap:16px;flex-wrap:wrap">
        <span style="color:${trendColor};font-size:14px">${trendIcon} Geçen haftaya göre <strong>${trendText} puan</strong></span>
    ...
    <div style="background:#1e293b;border-radius:16px;overflow:hidden;margin-bottom:16px">
      <div style="padding:16px 20px;border-bottom:1px solid #334155">
        <h3 style="color:#f1f5f9;margin:0;font-size:15px">👥 Ekip Performansı</h3>
      </div>
      <table style="width:100%;border-collapse:collapse">
        <thead>
          <tr style="background:#0f172a">
            <th style="padding:10px;text-align:left;color:#64748b;font-size:12px;font-weight:500">TEMSİLCİ</th>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
      <div style="background:#064e3b;border:1px solid #065f46;border-radius:12px;padding:16px">
        <div style="color:#10b981;font-size:12px;font-weight:600;margin-bottom:8px">🏆 EN İYİ KONUŞMA</div>
      ...
      <div style="background:#450a0a;border:1px solid #7f1d1d;border-radius:12px;padding:16px">
        <div style="color:#ef4444;font-size:12px;font-weight:600;margin-bottom:8px">⚠️ GELİŞTİRİLECEK</div>
    ...
    <div style="text-align:center;color:#334155;font-size:12px">
      <p>LeadFlow AI · Haftalık rapor otomatik gönderilmektedir</p>
      <p>Raporları durdurmak için <a href="#" style="color:#6d28d9">buraya tıklayın</a></p>
    </div>
  </div>
</body>
</html>`;
...
// POST /api/ti-reports/send-weekly — Manuel tetikle
router.post('/send-weekly', async (req: any, res: any) => {
  try {
    ...
    await transporter.sendMail({
      ...
      to: report.userEmail,
      subject: `📊 Haftalık Ekip Raporu — Ort. Skor: ${report.avgScore}`,
    ...
    res.json({ ok: true, message: `Rapor ${report.userEmail} adresine gönderildi`, avg_score: report.avgScore });
  } catch (e: any) {
    res.status(500).json({ error: e.message });
  }
});

// GET /api/ti-reports/preview — Email önizleme
router.get('/preview', async (req: any, res: any) => {
  try {
    const userId = req.userId;
    const report = await generateWeeklyReport(userId);
    if (!report) return res.status(400).json({ error: 'Veri bulunamadı' });
...
// GET /api/ti-reports/trend — Zaman trendi
...
// GET /api/ti-reports/benchmark — Sektör karşılaştırması
...
// POST /api/ti-reports/alert — Düşük skor uyarısı
router.post('/alert', async (req: any, res: any) => {
  try {
    const userId = req.userId;
    const { threshold = 50 } = req.body;

    // Son 24 saatteki düşük skorlu analizler
    ...
    const alertHtml = `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0f172a;padding:24px;border-radius:16px">
        <h2 style="color:#ef4444">⚠️ Düşük Performans Uyarısı</h2>
        <p style="color:#94a3b8">Son 24 saatte ${analyses.length} düşük skorlu konuşma tespit edildi:</p>
        ${analyses.map((a: any) => `
          <div style="background:#1e293b;border-left:3px solid #ef4444;padding:12px;margin:8px 0;border-radius:8px">
            ...
            <div style="color:#94a3b8;font-size:13px;margin-top:4px">${a.customer_phone} · ${a.channel}</div>
    ...
    await transporter.sendMail({
      from: `"LeadFlow AI" <${process.env.RESEND_FROM_EMAIL || 'onboarding@resend.dev'}>`,
      to: userEmail,
      subject: `⚠️ ${analyses.length} Düşük Performanslı Konuşma Tespit Edildi`,
      html: alertHtml,
    });

    res.json({ ok: true, sent: analyses.length, message: `${analyses.length} uyarı emaili gönderildi` });
  } catch (e: any) {
    res.status(500).json({ error: e.message });
  }
});

// GET /api/ti-reports/settings — Rapor ayarları
...
// PATCH /api/ti-reports/settings — Rapor ayarlarını güncelle
