...
const LANGUAGE_OPENINGS: Record<string, string> = {
  ...
  en: 'Hello! This is {{agent_name}} calling from {{company_name}}. Do you have a moment?',
  de: 'Guten Tag! Hier ist {{agent_name}} von {{company_name}}. Haben Sie einen Moment?',
  fr: 'Bonjour! Je suis {{agent_name}} de {{company_name}}. Avez-vous un moment?',
  ar: 'مرحبا! انا {{agent_name}} من شركة {{company_name}}. هل لديك دقيقة؟',
  ru: 'Zdravstvuyte! Eto {{agent_name}} iz {{company_name}}. Est minuta?',
  ...
  zh: 'Nin hao! Wo shi {{company_name}} de {{agent_name}}. Nin fangbian ma?',
  ja: 'Konnichiwa! {{company_name}} no {{agent_name}} to moushimasu. Yoroshii deshouka?',
  ko: 'Annyeonghaseyo! {{company_name}} ui {{agent_name}} imnida. Jamsi sidgan doesinayo?',
  hi: 'Namaste! Main {{agent_name}} hun, {{company_name}} se bol raha hun. Kya aap baat kar sakte hain?',
  pt: 'Ola! Aqui e {{agent_name}} da {{company_name}}. Tem um momento?',
  pl: 'Dzien dobry! Jestem {{agent_name}} z {{company_name}}. Czy ma Pan/Pani chwile?',
};
...
router.get('/eleven-voices', async (req: any, res: any) => {
  try {
    ...
    const norm = (v: any, src: string) => ({
      voice_id: v.voice_id,
      name: v.name,
      category: v.category || src,
      preview_url: v.preview_url || null,
      gender: (v.labels && v.labels.gender) || v.gender || null,
      accent: (v.labels && v.labels.accent) || v.accent || null,
      use_case: (v.labels && v.labels.use_case) || v.use_case || null,
      language: (v.labels && v.labels.language) || v.language || null,
      source: src,
    });
    ...
    res.json({
      categories: {
        my: myV,
        cloned: myV.filter((v: any) => v.category === 'cloned'),
        professional: myV.filter((v: any) => ['professional', 'customer_service', 'sales'].includes(v.use_case)),
        language: langV,
        all: [...myV, ...langV],
      },
...
router.post('/preview-voice', async (req: any, res: any) => {
  try {
    ...
    const defaults: Record<string, string> = {
      ...
      de: 'Guten Tag! Ich moechte Ihnen kurz etwas mitteilen.',
      ar: 'مرحباً، كيف حالك؟',
      fr: 'Bonjour! Je voudrais partager quelques informations.',
    };
    const r = await axios.post(
      `${ELEVEN_BASE}/text-to-speech/${voiceId}`,
      { text: text || defaults[language || 'tr'] || defaults['tr'], model_id: 'eleven_turbo_v2_5', voice_settings: { stability: 0.75, similarity_boost: 0.85 } },
      { headers: { 'xi-api-key': ELEVEN_KEY, 'Content-Type': 'application/json' }, responseType: 'arraybuffer', timeout: 15000 }
    );
...
router.post('/clone', upload.single('audio'), async (req: any, res: any) => {
  try {
    const { name } = req.body;
    const file = req.file;
    if (!file) return res.status(400).json({ error: 'Ses dosyasi zorunlu' });
    const form = new FormData();
    form.append('name', name || `LeadFlow-${req.userId.slice(0, 8)}`);
    form.append('description', 'LeadFlow AI satis sesi');
    ...
    await supabase.from('voice_settings').upsert([{ user_id: req.userId, elevenlabs_voice_id: voiceId, voice_name: name || 'Klonlanmis Sesim' }]);
    try { fs.unlinkSync(file.path); } catch {}
    res.json({ ok: true, voiceId, message: 'Ses klonlandi!' });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
router.post('/call/single', async (req: any, res: any) => {
  try {
    ...
    if (!lead.phone) return res.status(400).json({ error: 'Telefon numarasi yok' });
    ...
    const agentName = settings?.agent_name || userRow?.name || 'Satis Temsilcisi';
    ...
    res.json({ ok: true, callId: callRecord?.id, message: 'Arama baslatiliyor...' });
...
router.post('/call/campaign', async (req: any, res: any) => {
  try {
    ...
    res.json({ ok: true, campaignId: campaign?.id, total: leadIds.length, message: `${leadIds.length} lead icin arama baslatiliyor` });
...
router.post('/webhook/elevenlabs', async (req: any, res: any) => {
  try {
    const { conversation_id, transcript, analysis } = req.body;
    res.sendStatus(200);
