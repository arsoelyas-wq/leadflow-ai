    ...
    try {
      ...
      if (aiM) {
        ...
        employees = (aiData.persons || []).map((p: any) => ({
          ...
          aiAnalysis: {
            isDecisionMaker: true,
            decisionPower: 'yuksek',
            personalizedOpener: p.personalizedOpener || `Merhaba, ${lead.company_name} ile gorusmek istiyordum`,
            approachStrategy: p.approachStrategy || 'Dogrudan yaklas',
          }
        }));
        console.log('AI prediction result:', employees.length, 'persons');
      }
    } catch(aiErr: any) { console.error('AI prediction error:', aiErr.message); }
