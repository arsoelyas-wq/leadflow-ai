...
function wrapLinks(text: string, trackingMap: Record<string, string>): string {
  const urlRegex = /(https?:\/\/[^\s]+)/g;
  return text.replace(urlRegex, (url) => {
    if (trackingMap[url]) return trackingMap[url];
    return url;
  });
}
...
router.get('/redirect/:code', async (req: any, res: any) => {
  try {
    ...
    if (error || !link) return res.status(404).send('Link bulunamadı');
...
router.get('/stats', async (req: any, res: any) => {
  try {
    ...
    const totalClicks = (data || []).reduce((sum: number, l: any) => sum + (l.clicks || 0), 0);
