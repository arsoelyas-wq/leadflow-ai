...
router.get('/', async (req: any, res: any) => {
  try {
    ...
    const totalSent = campaigns?.reduce((sum: number, c: any) => sum + (c.total_sent || 0), 0) || 0;
    const totalReplied = campaigns?.reduce((sum: number, c: any) => sum + (c.total_replied || 0), 0) || 0;
