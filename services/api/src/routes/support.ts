...
function buildDashboardSystemPrompt(user: any, pageContext?: string): string {
...
async function callClaude(systemPrompt: string, messages: any[]): Promise<{ message: string; quickReplies: string[]; needsEscalation: boolean }> {
...
router.post('/public/chat', async (req: any, res: any) => {
  try {
...
router.post('/conversations/:id/messages', async (req: any, res: any) => {
  try {
    ...
    if (!content?.trim()) return res.status(400).json({ error: 'Mesaj içeriği boş' });
