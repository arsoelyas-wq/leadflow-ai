...
function getDistricts(city, maxResults) {
    if (maxResults <= 100)
        return [city];
...
function isValidEmail(email) {
    ...
    if (EMAIL_BLACKLIST.has(domain))
        return false;
    if (domain.includes('.jpg') || domain.includes('.png'))
        return false;
    if (/\d{4,}/.test(domain))
        return false; // numeric-heavy domains are usually artifacts
    return true;
}
function rankEmail(email) {
    const local = email.split('@')[0].toLowerCase();
    if (['info', 'iletisim', 'contact', 'hello', 'mail', 'sales', 'hola'].includes(local))
        return 10;
    if (['bilgi', 'destek', 'support', 'office'].includes(local))
        return 8;
    if (local.length < 15)
        return 5; // shorter locals are usually real
    return 2;
}
...
async function enrichWithEmails(leads, onProgress) {
    const CONCURRENCY = 6;
    let enriched = 0;
...
function calcScore(place, emailFound) {
    let score = 0;
    // Phone validity (+20)
    ...
    if (place.primaryTypeDisplayName || place.types?.length > 0)
        score += 5;
    // Has photos (+5)
...
async function fetchPlacesPage(opts) {
    ...
    if (!resp.ok)
        throw new Error(`Places API error: ${await resp.text()}`);
...
async function scrapeLeads(opts) {
    const { keyword, city, country, maxResults, userId, minScore = 0, requirePhone = false, requireWebsite = false, discoverEmails = false, onProgress, } = opts;
    ...
    const { data: existing } = await supabase
        ...
        .eq('user_id', userId)
        .limit(3000);
    const existingNames = new Set((existing || []).map((l) => (l.company_name || '').toLowerCase().trim()));
    ...
    outer: for (const district of districts) {
        ...
        for (const kw of keywords) {
            ...
            do {
                ...
                try {
                    ...
                    for (const p of places) {
                        ...
                        const phone = validatePhone(p.nationalPhoneNumber || p.internationalPhoneNumber || '', country);
                        ...
                        if (requireWebsite && !p.websiteUri)
                            continue;
                        seenIds.add(p.id);
            ...
            queryIdx++;
            if (queryIdx % 4 === 0 && collected.length < maxResults)
                await sleep(300);
        }
    }
    ...
    if (discoverEmails && collected.length > 0) {
        ...
        await enrichWithEmails(collected, (enrichedN) => {
    ...
    for (let i = 0; i < leadsToInsert.length; i += 50) {
        ...
        if (error) {
            ...
            if (!insertError)
                insertError = `Veritabanı hatası: ${error.message}`;
        }
        if (data)
...
router.post('/google-maps', async (req, res) => {
    try {
        ...
        const { keyword, city, country = 'TR', maxResults = 20, minScore = 0, requirePhone = false, requireWebsite = false, discoverEmails = false, } = req.body;
        if (!keyword || !city) {
            ...
            scrapeLeads({
                ...
                onProgress: (phase, found, saved, enriched) => {
                    ...
                    if (j) {
                        ...
                        j.enriched = enriched;
                        if (found > j.total)
                            j.total = found; // dynamic update if more found
                    }
                },
                ...
                if (j) {
                    ...
                    j.saved = count;
                    j.withPhone = stats.withPhone;
                    j.withEmail = stats.withEmail;
                    j.withWebsite = stats.withWebsite;
                    if (skipped > 0)
                        j.phase = `${count} kaydedildi, ${skipped} tekrar atlandı`;
                    if (count === 0 && apiError)
...
router.get('/test-key', async (req, res) => {
    ...
    const q = req.query.q || 'coffee Istanbul Turkey';
    try {
        ...
        return res.json({
            ...
            samples: (data.places || []).slice(0, 3).map((p) => p.displayName?.text),
        });
    }
    catch (e) {
