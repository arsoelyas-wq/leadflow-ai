...
router.get('/segments', async (req, res) => {
    try {
        ...
        if (max_score)
            query = query.lte('score', Number(max_score));
        if (city)
        ...
        if (error)
            throw error;
        res.json({ leads: data || [], total: count || 0, filters: { cities: [...new Set((data || []).map((l) => l.city).filter(Boolean))], sectors: [...new Set((data || []).map((l) => l.sector).filter(Boolean))], sources: [...new Set((data || []).map((l) => l.source).filter(Boolean))] } });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.get('/analytics', async (req, res) => {
    try {
        ...
        const hourlyDist = {};
        const hourlyReply = {};
        out.forEach((m) => { const h = new Date(m.sent_at).getHours(); hourlyDist[h] = (hourlyDist[h] || 0) + 1; });
        inc.forEach((m) => { const h = new Date(m.sent_at).getHours(); hourlyReply[h] = (hourlyReply[h] || 0) + 1; });
        let bestHour = 9, bestRate = 0;
        for (let h = 8; h <= 20; h++) {
            const s = hourlyDist[h] || 0;
            const r = hourlyReply[h] || 0;
            if (s > 0) {
                const rate = Math.round((r / s) * 100);
                if (rate > bestRate) {
                    bestRate = rate;
                    bestHour = h;
                }
        ...
        const byChannel = {};
        out.forEach((m) => { if (!byChannel[m.channel])
            byChannel[m.channel] = { sent: 0, replied: 0 }; byChannel[m.channel].sent++; });
        inc.forEach((m) => { if (byChannel[m.channel])
            byChannel[m.channel].replied++; });
        res.json({ totalCampaigns: camps?.length || 0, totalSent: out.length, totalReplied: inc.length, replyRate: out.length > 0 ? Math.round((inc.length / out.length) * 100) : 0, bestHour: `${String(bestHour).padStart(2, '0')}:00`, byChannel: Object.entries(byChannel).map(([ch, v]) => ({ channel: ch, ...v, replyRate: v.sent > 0 ? Math.round((v.replied / v.sent) * 100) : 0 })), campaigns: (camps || []).map((c) => ({ ...c, replyRate: c.total_sent > 0 ? Math.round(((c.total_replied || 0) / c.total_sent) * 100) : 0 })) });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
});
router.get('/smart-timing', async (req, res) => {
    try {
        ...
        const hs = {};
        for (let h = 7; h <= 21; h++)
            hs[h] = { sent: 0, replied: 0 };
        out.forEach((m) => { const h = new Date(m.sent_at).getHours(); if (hs[h])
            hs[h].sent++; });
        inc.forEach((m) => { const h = new Date(m.sent_at).getHours(); if (hs[h])
            hs[h].replied++; });
...
router.get('/:id/funnel', async (req, res) => {
    try {
        ...
        for (const m of (messages || [])) {
            if (m.direction === 'out') {
                sentLeads.add(m.lead_id);
                funnel.sent++;
                if (m.status === 'delivered' || m.status === 'read')
                    funnel.delivered++;
            }
            else {
                repliedLeads.add(m.lead_id);
                funnel.replied++;
            }
        }
        for (const e of (events || [])) {
            if (e.event_type === 'email_opened')
                openedLeads.add(e.lead_id);
        }
