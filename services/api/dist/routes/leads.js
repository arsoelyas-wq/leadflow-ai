...
async function logFieldChange(leadId, userId, field, oldVal, newVal, action = 'update') {
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        if (search)
            query = query.or(`company_name.ilike.%${search}%,phone.ilike.%${search}%,contact_name.ilike.%${search}%`);
        if (min_score)
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        const { company_name, contact_name, phone, email, website, city, sector, source, notes, score, status, deal_value, win_probability, tags, utm_source, utm_medium, utm_campaign, utm_content, utm_term, fbc, fbp, force = false, } = req.body;
        if (!company_name)
        ...
        if (!force) {
            const dupChecks = [];
            if (phone)
                dupChecks.push(supabase.from('leads').select('id,company_name,phone').eq('user_id', req.userId).eq('phone', phone).is('deleted_at', null).maybeSingle());
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        // Audit log
        if (lead)
            await logFieldChange(req.params.id, req.userId, 'deleted_at', null, new Date().toISOString(), 'delete');
...
router.post('/import', authMiddleware, upload.single('file'), async (req, res) => {
    try {
        ...
        if (rows.length > 2000)
            return res.status(400).json({ error: 'Tek seferde maksimum 2.000 lead yüklenebilir' });
        ...
        for (let i = 0; i < rows.length; i++) {
            const raw = rows[i];
            const lead = { user_id: req.userId, source: 'Excel Import', status: 'new', score: 50, created_at: now, updated_at: now };
            for (const [rawKey, rawVal] of Object.entries(raw)) {
                ...
                if (field && rawVal != null && String(rawVal).trim()) {
                    ...
                    if (field === 'score') {
                        const s = parseInt(v);
                        if (!isNaN(s))
                            lead.score = Math.max(0, Math.min(100, s));
                    }
                    else {
                        lead[field] = v;
                    }
        ...
        for (let b = 0; b < toInsert.length; b += BATCH) {
            ...
            if (!insErr)
                ...
                console.warn('[Import] Batch error:', insErr.message?.slice(0, 80));
        }
...
router.get('/import/template', authMiddleware, (_req, res) => {
...
router.get('/:id/next-action', authMiddleware, async (req, res) => {
    try {
...
Son aktiviteler: ${(history || []).slice(0, 3).map((h) => `${h.field_name}: ${h.new_value}`).join(', ')}
...
NEDEN: [1 cümle neden]`
        ...
        res.json({ action: 'Müşteriyi ara veya WhatsApp mesajı gönder', timing: 'Bugün', reason: 'AI analizi geçici olarak kullanılamıyor' });
    }
});
...
router.get('/pipeline/stats', authMiddleware, async (req, res) => {
    try {
        ...
        for (const l of all) {
            ...
            if (l.deal_value)
                totalValue += Number(l.deal_value);
            if (l.status === 'won') {
                wonValue += Number(l.deal_value || 0);
                wonCount++;
            }
        }
...
router.post('/bulk-enrich', authMiddleware, async (req, res) => {
    try {
        const { ids } = req.body;
        if (!ids?.length)
            return res.status(400).json({ error: 'ids gerekli' });
        if (ids.length > 100)
            return res.status(400).json({ error: 'Tek seferde max 100 lead zenginleştirilebilir' });
        ...
        setImmediate(async () => {
            for (const id of ids) {
                try {
                    ...
                    if (typeof triggerEnrichment === 'function')
                        await triggerEnrichment(supabase, lead, req.userId);
                    else {
...
router.post('/check-duplicate', authMiddleware, async (req, res) => {
    try {
        ...
        const checks = [];
        if (phone)
            checks.push(supabase.from('leads').select('id,company_name,phone,status').eq('user_id', req.userId).eq('phone', phone).is('deleted_at', null).limit(1));
        if (company_name)
            checks.push(supabase.from('leads').select('id,company_name,phone,status').eq('user_id', req.userId).ilike('company_name', company_name.trim()).is('deleted_at', null).limit(1));
...
router.delete('/segments/:id', authMiddleware, async (req, res) => {
    try {
...
router.post('/custom-fields', authMiddleware, async (req, res) => {
    try {
        ...
        if (!field_key || !field_label)
            return res.status(400).json({ error: 'field_key ve field_label zorunlu' });
...
router.get('/:id/custom-values', authMiddleware, async (req, res) => {
    try {
        const { data } = await supabase.from('lead_custom_values').select('*').eq('lead_id', req.params.id);
        const vals = {};
        (data || []).forEach((r) => { vals[r.field_key] = r.value; });
...
router.patch('/:id/custom-values', authMiddleware, async (req, res) => {
    try {
        ...
        if (upserts.length)
            await supabase.from('lead_custom_values').upsert(upserts, { onConflict: 'lead_id,field_key' });
        res.json({ success: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
