...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        if (!user)
            return res.json({ ok: true, message: 'Eğer bu email kayıtlıysa, sıfırlama bağlantısı gönderildi.' });
        // Stateless token: signed with JWT_SECRET + current password_hash (invalidated on password change)
        const resetToken = jwt.sign({ userId: user.id, purpose: 'pw_reset' }, (process.env.JWT_SECRET || '') + user.password_hash, { expiresIn: '1h' });
...
router.post('/send-otp', async (req, res) => {
    try {
        ...
        if (!user)
            return res.json({ ok: true, masked_email: null });
...
router.post('/verify-otp', async (req, res) => {
    try {
        ...
        if (!user.otp_code)
            return res.status(401).json({ error: 'Önce kod isteyin' });
