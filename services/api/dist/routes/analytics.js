...
router.get('/revenue', async (req, res) => {
    try {
        ...
        const [{ data: allLeads }, { data: recentMessages }, { data: campaigns }, { data: user },] = await Promise.all([
        ...
        for (let i = 12; i >= 0; i--) {
            ...
            weeklyData[key] = {
                ...
                messages: messages.filter((m) => m.direction === 'out' && new Date(m.sent_at) >= weekStart && new Date(m.sent_at) < weekEnd).length,
                replies: messages.filter((m) => m.direction === 'in' && new Date(m.sent_at) >= weekStart && new Date(m.sent_at) < weekEnd).length,
            };
        }
        // ── FUNNEL DÖNÜŞÜM ORANLARI ───────────────────────────
        const totalLeads = leads.length;
        const contacted = leads.filter((l) => ['contacted', 'qualified', 'won', 'lost'].includes(l.status)).length;
        ...
        const last30Leads = leads.filter((l) => l.created_at >= thirtyDaysAgo).length;
        const last30Messages = messages.filter((m) => m.direction === 'out' && m.sent_at >= thirtyDaysAgo).length;
        const last30Replies = messages.filter((m) => m.direction === 'in' && m.sent_at >= thirtyDaysAgo).length;
        // Önceki 30 gün
        ...
        // ── KANAL PERFORMANSI ─────────────────────────────────
        const waMessages = messages.filter((m) => m.channel === 'whatsapp' && m.direction === 'out').length;
        const waReplies = messages.filter((m) => m.channel === 'whatsapp' && m.direction === 'in').length;
        const emailMessages = messages.filter((m) => m.channel === 'email' && m.direction === 'out').length;
        const emailReplies = messages.filter((m) => m.channel === 'email' && m.direction === 'in').length;
        ...
        if (replyRate30 < 5)
            recommendations.push('Mesaj şablonlarınızı kişiselleştirin — cevap oranınız düşük (%' + replyRate30 + ')');
        if (leadGrowth < 0)
            recommendations.push('Lead büyümeniz yavaşlıyor — rakip hijacking veya yeni scraping deneyin');
        if (winRate < 10)
            recommendations.push('Satış kapama oranınız düşük — nitelikli lead\'lere öncelik verin');
        if (waMessages === 0)
            recommendations.push('WhatsApp kampanyası başlatmadınız — en yüksek dönüşüm kanalı!');
        if (last30Leads > 50 && contactRate < 30)
            recommendations.push('Çok lead var ama iletişime geçilmiyor — kampanya başlatın');
        if (recommendations.length === 0)
            recommendations.push('Harika gidiyorsunuz! Büyümeyi sürdürmek için aylık lead hedefini artırın');
        res.json({
            // Özet
            summary: {
                last30Leads,
...
router.get('/financial', async (req, res) => {
    try {
        ...
        leads.forEach(lead => {
            ...
            if (!sourceStats[key])
                sourceStats[key] = { total: 0, won: 0, score: 0 };
...
3 madde finansal büyüme tavsiyesi ver. JSON: {"advice": ["tavsiye1","tavsiye2","tavsiye3"], "priority": "en önemli eylem", "forecast": "önümüzdeki ay tahmin"}`
            ...
            if (match)
                financialAdvice = JSON.parse(match[0]);
        }
        catch { }
        res.json({
            monthlyTrend,
