...
router.get('/', async (req, res) => {
    try {
        ...
        const [{ count: totalLeads }, { count: weekLeads }, { count: prevWeekLeads }, { count: activeCampaigns }, { count: totalCampaigns }, { data: campaigns }, { data: userCredits }, { data: recentLeads }, { data: recentCampaigns }, { data: leadsByStatus }, { data: wonLeads }, { data: recentActivity },] = await Promise.all([
        ...
        const totalSent = campaigns?.reduce((s, c) => s + (c.total_sent || 0), 0) || 0;
        const totalReplied = campaigns?.reduce((s, c) => s + (c.total_replied || 0), 0) || 0;
        ...
        const statusCounts = {};
        (leadsByStatus || []).forEach((l) => { statusCounts[l.status] = (statusCounts[l.status] || 0) + 1; });
        const funnel = [
