...
function detectLineType(phone, country) {
...
async function smtpPing(email, mxHost) {
...
function validatePhone(phone) {
    if (!phone)
        return { valid: false, formatted: '', country: '', lineType: 'unknown', score: 0, reason: 'Telefon yok' };
    ...
    else if (/^(\+1|001)/.test(cleaned))
        country = 'US';
    else if (/^(\+49|0049)/.test(cleaned))
        country = 'DE';
    else if (/^(\+44|0044)/.test(cleaned))
        country = 'GB';
    else if (/^(\+33|0033)/.test(cleaned))
        country = 'FR';
    else if (/^(\+31|0031)/.test(cleaned))
        country = 'NL';
    else if (/^(\+971|00971)/.test(cleaned))
        country = 'AE';
    else if (/^(\+966|00966)/.test(cleaned))
        country = 'SA';
    else if (/^(\+7|007)/.test(cleaned))
        country = 'RU';
...
// ── EMAIL DOĞRULAMA ───────────────────────────────────────
async function validateEmail(email, withSmtp = false) {
    if (!email)
        return { valid: false, score: 0, reason: 'Email yok' };
...
// ── ANA SKOR HESAPLAMA ────────────────────────────────────
async function calculateLeadScore(lead, withSmtp = false) {
    const breakdown = {};
    let totalScore = 0;
...
router.post('/validate', async (req, res) => {
    try {
        ...
        const result = await calculateLeadScore({ phone, email, website, company_name, city, sector, rating, review_count }, smtp === true);
        res.json(result);
    }
    catch (e) {
...
router.post('/batch', async (req, res) => {
    try {
        ...
        if (error)
            throw error;
        if (!leads?.length)
            return res.json({ message: 'Lead bulunamadı', updated: 0 });
        let updated = 0;
        const results = [];
