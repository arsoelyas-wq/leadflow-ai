...
if (process.env.NODE_ENV !== 'production') {
    app.get('/api/scrape/test-key', (req, res, next) => { req.url = '/test-key'; require('./routes/scrape')(req, res, next); });
    app.get('/api/lead-finder/test-apify', (req, res, next) => { req.url = '/test-apify'; lfRouter(req, res, next); });
    app.get('/api/decision-maker/test-linkedin', (req, res, next) => { req.url = '/test-linkedin'; dmRouter(req, res, next); });
}
// PROTECTED
...
app.post('/api/payments/webhook', express.raw({ type: 'application/json' }), (req, res, next) => { req.url = '/webhook'; paymentsRouter(req, res, next); });
...
app.get('/api/google-ads/callback', (req, res, next) => { req.url = '/callback'; googleAdsRouter(req, res, next); });
...
// TwiML test — DEV only
if (process.env.NODE_ENV !== 'production') {
    app.get('/api/twiml/test', function (_req, res) { res.setHeader('Content-Type', 'text/xml'); res.send('<Response><Say language="tr-TR" voice="Polly.Filiz">Test modu</Say></Response>'); });
}
app.post('/api/team-intelligence/process-call', (req, res, next) => { req.url = '/process-call'; tiRouter(req, res, next); });
...
app.post('/api/green-api/webhook', (req, res, next) => { req.url = '/webhook'; greenApiRouter(req, res, next); });
app.post('/api/green-api/connected', (req, res, next) => { req.url = '/connected'; greenApiRouter(req, res, next); });
...
global.triggerWorkflowEvent = triggerWorkflowEvent;
...
global.processIncomingWhatsApp = _piMsg;
...
app.get('/api/leads-config', authMiddleware, async (_req, res) => {
    try {
        const { createClient } = require('@supabase/supabase-js');
        const sb = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
